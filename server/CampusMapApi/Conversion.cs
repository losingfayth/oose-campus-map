...
public class CoordinateConverter
{
        ...
        int length)
    {
        ...
        public double Get_NorthLatitude() { return _northLatitude; }

        public double Get_EastLongitude() { return _eastLongitude; }
    ...
    public static double GetDistance(string openLocationCode1, string openLocationCode2, Coordinate.DistanceMetric metric) {
        return Coordinate.GetDistance(
            OpenLocationCode.DecodeToCenter(openLocationCode1), OpenLocationCode.DecodeToCenter(openLocationCode2), 
            metric
        );
    }
    ...
    public static void ExampleUsage() {
        ...
        // Note the negative signs - the longitude is given relative to WEST,
        // so we need the negative signs. North and East are positive, South
        ...
        double distance = Coordinate.GetDistance(
            coordinate,
            coordinateB, 
            Coordinate.DistanceMetric.Miles
        );
        // ...where the last argument gives the units you want the distance
        // returned in.
        // Miles, feet, meters, and kilometers are valid options.

        // Most (all) the points we're worried about are in Bloomsburg.
        // For example, Ben Franklin Hall is apprx. 41°00'24.8"N 76°26'53.7"W

        string benFranklinCode = 
            OpenLocationCode.EncodeInBloom(0, 24.8, -26, -53.7);

        // You can skip adding the degrees for lat/lng - the minutes and seconds are enough.

        // A Plus Code with 11 digits refers to (approximately) a 3x3 meter 
        // area. That is the default precision.
        // Given a plus code, you can get access to the rectangular bounds of 
        // that area.
        ...
        center = 
            new (codeArea.GetCenterLatitude(), codeArea.GetCenterLongitude());

        // Since all Plus codes we'll be dealing with are in Bloom, they share 
        ...
        string fullCode = 
            OpenLocationCode.AppendBloomPrefix("2H42+WJF").GetCode();
        // ...and then decoded
        center = 
            OpenLocationCode.DecodeToCenter(fullCode);
        // ...or...
        center = 
            OpenLocationCode.AppendBloomPrefix("2H42+WJF").DecodeToCenter();
    ...
    public static void TestDistanceFunction() {
        ...
        // Should be around 400 feet
        Console.WriteLine(Coordinate.GetDistance(c1, c2)); 

        c1 = new(41.084828, -76.863956); 
        
        // Should be around 14 miles
        Console.WriteLine(Coordinate.GetDistance(
            c1, c2, Coordinate.DistanceMetric.Miles
        ));
    ...
    public static void TestBloomEncoding()
    {
        ...
        string[] validEncodings = 
            [OpenLocationCode.AppendBloomPrefix("2H42+544").GetCode()];

        for (int i = 0; i < minSecLatLngPairs.GetLength(0); i++)
        {
            string encoding = OpenLocationCode.EncodeInBloom(
                minSecLatLngPairs[i, 0], minSecLatLngPairs[i, 1], minSecLatLngPairs[i, 2], minSecLatLngPairs[i, 3]
            );

            string encodingB = OpenLocationCode.Encode(
                Coordinate.GetBloomsburgCoordinates(minSecLatLngPairs[i, 0], minSecLatLngPairs[i, 1], minSecLatLngPairs[i, 2], minSecLatLngPairs[i, 3]), 11
            );
            
            Console.Write(encoding + " | " + encodingB + " | " 
                + validEncodings[i] + " | ");

            if (string.Compare(encoding, validEncodings[i]) == 0 
                && string.Compare(encoding, encodingB) == 0)
            { Console.Write("Pass\n"); }
    ...
    {
        double[,] latlngPairs = { 
            { 41.011846, -76.444027, 10 }, 
            { 41.011846, -76.444027, 11 } 
        };
        ...
        {
            string encoding = OpenLocationCode.Encode(
                latlngPairs[i, 0], 
                latlngPairs[i, 1], 
                (int)latlngPairs[i, 2]
            );
